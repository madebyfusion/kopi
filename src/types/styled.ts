...
import type { Twind, StyleConfig, StyleProps } from 'twind';

import isPropValid from '@emotion/is-prop-valid';
import { style, tw as defaultTW } from 'twind';
...
export function styled<Variants, BaseVariants, Tag extends React.ElementType>(
  this: Twind | null | undefined | void,
...
export function styled<Tag extends React.ElementType, Variants>(
  ...
  tag: Tag = 'div' as Tag,
  {
    ...
    ...config
  }: StyledOptions & StyleConfig<Variants> = {},
) {
  const tw = typeof this == 'function' ? this : defaultTW;

  const component = style(config);

  const validateProp = (prop: string): boolean =>
    shouldForwardProp(prop, isPropValid);
  ...
  const sc = React.forwardRef(
    <T extends React.ElementType = Tag>(
      ...
      React.createElement(as, {
        ...
        ref,
        className: tw(
          component((typeof as == 'string' ? { ...props, as } : props) as any),
        ),
      }),
  );
